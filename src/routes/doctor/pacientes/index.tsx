...
// ─── Tipos que reflejan la API real ────────────────────────────────────────────
type PacienteApi = {
  id: number
  nombre: string
  edad: number
...
const PACIENTES_RECIENTES_LIMITE = 5
...
function RouteComponent() {
  ...
  const { data: pacientes, isLoading: isLoadingPacientes } = useQuery({
    queryKey: ['pacientes'],
    queryFn: async () => {
      const res = await fetchWithToken<{ pacientes: PacienteApi[] }>(`${API_BASE_URL}/api/pacientes`, {
        headers: { 'content-type': 'application/json' },
        method: 'GET'
      })
      return res.data?.pacientes as unknown as PacienteApi[]
    },
  })

  const { data: historial } = useQuery({
    queryKey: ['consulta-historial-total'],
    queryFn: async () => {
      const res = await fetchWithToken<HistorialResponse>(
        `${API_BASE_URL}/api/consulta/historial?page=1&pageSize=1`
      )
      return res.data
    },
  })
  ...
  const totalConsultas = historial?.total ?? 0

  // Filtro de búsqueda sobre los pacientes más recientes
  const pacientesFiltrados = useMemo(() => {
    ...
    return filtrados.slice(0, PACIENTES_RECIENTES_LIMITE)
  }, [search, pacientes])
    ...
    <MainPanel>
      ...
      <div className="grid grid-cols-2 gap-4 pt-4">
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white/80 p-5 shadow-[0_2px_12px_rgba(15,23,42,0.06)] backdrop-blur-sm">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400 mb-1.5">Consultas de IA Registradas</p>
      ...
      <div className="grid grid-cols-3 gap-4 pt-4">
        ...
        <div className="col-span-2 rounded-2xl border border-slate-200/80 bg-white/80 shadow-[0_2px_12px_rgba(15,23,42,0.06)] backdrop-blur-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            <h2 className="text-[14px] font-bold text-slate-800">Pacientes Recientes</h2>
            <div className="flex gap-2">
              <Link
                to="/doctor/pacientes"
                className="text-[12px] font-semibold text-slate-500 border border-slate-200 px-3 py-1.5 rounded-lg hover:bg-slate-50 transition-colors"
              >
                Ver Todos
              </Link>
              <Link
          ...
          <table className="w-full">
            <thead className="bg-slate-50/80">
              <tr>
                {['Edad', 'Nombre del Paciente', 'Documento', 'Sexo'].map(h => (
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoadingPacientes ? (
                ...
                const iniciales = p.nombre.split(' ').map(part => part[0]).slice(0, 2).join('').toUpperCase()
                return (
                  <tr key={p.id} className="hover:bg-slate-50/60 transition-colors">
                    <td className="px-5 py-3.5 text-[12px] font-semibold text-slate-400">{p.edad}</td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-full bg-[#1565d8]/10 text-[#1565d8] flex items-center justify-center text-[11px] font-bold shrink-0">{iniciales}</div>
